...
@Repository
public class AddressDaoImpl extends AbstractDaoSupport implements AddressDao {
	...
	public Set<Address> getAddressListByUser(User user)
		...
		final String sql="from Address where user.userId=? order by createdOnDate asc";
		Query query = createQuery(sql,-1,-1,user.getUserId());
		return new LinkedHashSet<Address>(query.list());
	}
